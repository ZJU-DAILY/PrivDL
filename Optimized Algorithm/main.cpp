...
#include "degree_collection_opt.cpp"
#include "post_processing.cpp"

int main()
{
    std::vector<std::vector<std::pair<std::vector<int>, std::vector<int>>>> neighbor_list;
    std::string filename = "../Wiki-talk_Temporal_Network/wiki_talk_temporal";
    int T = 317;
    int node_num = read_graph(neighbor_list, filename, T);
    int window_size = 5;
    int theta = 15;
    //mod when dataset change
    std::pair<int, int> d_max = std::make_pair(1120, 44);

    double epsilon;
    


    for (epsilon = 2.5; epsilon >= 0.5; epsilon -= 0.5)
    {

        double epsilon_one = 0.1 * epsilon;
        double epsilon_two = 0.1 * epsilon;
        double epsilon_three = 0.1 * epsilon;
        double epsilon_four = epsilon - epsilon_one - epsilon_two - epsilon_three;    
        ...
        for (int repeat = 0; repeat < repeat_time; repeat++)
        {
            ...
            //used for every user
            std::vector<std::pair<std::vector<int>, std::vector<int>>> degree_list_each_user;

            for (t = 0; t < T; t++)
            {
                auto start = std::chrono::steady_clock::now();
                std::pair<std::vector<int>, std::vector<int>> degree_list;
                degree_collection_opt(degree_list, neighbor_list[t], d_max.first, d_max.second, node_num, window_size, epsilon_one, epsilon_two, epsilon_three, epsilon_four, theta, degree_list_each_user, t);
        ...
        std::cout << "step:" << theta << std::endl;
        std::cout << "epsilon:" << epsilon << std::endl;
        std::cout << "epsilon1:" << epsilon_one << std::endl;
        std::cout << "epsilon2:" << epsilon_two << std::endl;
        std::cout << "epsilon3:" << epsilon_three << std::endl;
        std::cout << "epsilon4:" << epsilon_four << std::endl;
